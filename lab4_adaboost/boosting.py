...
class WeakClassifier:
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray):
        ...
        self._dim = np.random.choice(d) # d: dim delle features (0, 1) -> 2
        _features_dim = X[:, self._dim] # vettore della features scelta con np.random.choice(d), X[:, axis] prende tutti gli elementi del vettore determinato un axis       

        """ pick a random threshold (see np.random.uniform)
            NB: look at the interval [min,max] from the selected dimension """
        self._threshold = np.random.uniform(np.min(_features_dim), np.max(_features_dim))

        """ pick a random verse (see np.random.choice)
            case a) feature >= _threshold ==>> then predict 1
            case b) feature >= _threshold ==>> then predict -1 """
        self._label_above_split = np.array([possible_labels[0] if i < self._threshold else possible_labels[1] for i in _features_dim])
    ...
    def predict(self, X: np.ndarray):
        ...
        y_pred = self._label_above_split
...
class AdaBoostClassifier:
    ...
    def __init__(self, n_learners: int, n_max_trials: int = 200):
        ...
        self.n_max_trials = n_max_trials #dopo n_max_trials tentativi se errore>0.5 azzera i pesi
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray, verbose: bool = False):
        ...
        n, d = X.shape # n = num esempi; d = num features per example
        possible_labels = np.unique(Y)
        ...
        """ initialize the sample weights as equally probable everyone with p = 1/n """
        ...
        for l in range(self.n_learners):
            ...
            #X.shape[0] da range di sampling [0:700], n è il num el. del new array, 
            # replace=True permette il campionamento ripetuto degli elementi, p = sample_weights da la prob per ogmi eleemento di essere campionato
            cur_idx = np.random.choice(X.shape[0], n, replace=True, p = sample_weights) 
            #ho vettore di indic campionati a caso fra 0:700
            # ora creo il mio new dataset di dim X.shape[0] pescando gli elementi da X secondo la disposizione randomica di cur_idx
            # extract 'difficult' samples, perchè andando avanti gli elementi più sbagliati avranno aumento di peso
            cur_X = X[cur_idx] 
            cur_Y = Y[cur_idx]
            ...
            if sample_weights[y_pred != cur_Y]:
