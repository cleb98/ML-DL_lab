...
class SVM:
    ...
    def loss(self, y_true, y_pred):
        ...
        """
        regularization = (self._lambda/2)*norm(self._w)
        hinge_los = np.mean(np.maximum(np.zeros(y_true.shape), 1- y_pred * y_true ))
        L = regularization + hinge_los
        return L
