...
def loss(y_true, y_pred):
    ...
    """
    first_sum=y_true*np.log(y_pred)
    second_sum=(1-y_true)*np.log(1-y_pred)
    return -1/y_true.shape[0]*np.sum(first_sum+second_sum)
...
def dloss_dw(y_true, y_pred, X):
    ...
    return -(np.dot(X_transposed,(y_true-y_pred)))/N
...
class LogisticRegression:
    ...
    def fit_gd(self, X, Y, n_epochs, learning_rate, verbose=False):
        ...
        for e in range(n_epochs):
            ...
            # -> self.w = ...
            """
            dloss=dloss_dw(Y,prediction,X)
            self.w=self.w-(learning_rate*dloss)
