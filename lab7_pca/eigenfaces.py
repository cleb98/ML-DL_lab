...
class Eigenfaces:
    ...
    def fit(self, X: np.ndarray, verbose: bool = False):
        ...
        self._x_mean = np.mean(X, axis = 0) #media rispetto a features quindi fisso la riga (axis = 0)
        ...
        cov = np.matmul(X_norm, np.transpose(X_norm))
        #cov= X_norm @ X_norm.T
        # compute eigenvectors of the covariance matrix
        eigval, eigvec = np.linalg.eig(cov)
        
        # sort them (decreasing order w.r.t. eigenvalues)
        indexes = eigval.argsort()[::-1] #ordina in modo decrescente
        sorted_eigval, sorted_eigvec = eigval[indexes], eigvec[:, indexes]
        ...
        self._ef = np.matmul(X.transpose(), principal_sorted_eigvec) #matrice n°features*n°.dim ridotte = n°autovettori 
    ...
    def transform(self, X: np.ndarray):
        ...
        """
        self.project = np.matmul(X, self._ef)
        # project faces according to the computed directions
        #return X @ self._ef
        return self.project

    def inverse_transform(self, X: np.ndarray):
        return np.matmul(self.project, self._ef.transpose())
